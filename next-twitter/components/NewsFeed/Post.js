...
const Post = ({ post, id }) => {
  ...
  const heartIcon =  hasLiked ? 
              ...
              /> <span className="text-[#1d9bf0]">{likes.length !== 0 && likes.length}</span></>) :
              (<><HeartIcon
  ...
  const deletePost = () => {
    if(window.confirm("Are you sure you want to delete this post?")){
      ...
      deleteDoc(doc(db,"posts",id,"likes"));
    if(post?.data()?.image){
          ...
          <div className="flex justify-between w-[80%] cursor-pointer">
            ...
            </div>
            {
              session?.user?.uid === post.data().id && (
                    <div className="flex items-center space-x-2 hover:text-[#1d9bf0] group" onClick={deletePost}>
